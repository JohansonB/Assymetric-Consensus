...
public class RecPbftClient extends ConditionalGenericProtocol {
    private static final File OUTPUT_DIR = new File("output");
    private static final short REPLICAID = RecPbftReplica.PROTO_ID;
    private static final String REPLICANAME = RecPbftReplica.PROTO_NAME;

    private static final String PROTO_NAME = "RecPbftClient";
    public static final short PROTO_ID = 400;
    public static final double DELTA = 200;
    public static final int NUM_REPETITIONS = 1000;
    public static final BenchmarkingChannel.Distribution DISTRIBUTION = new BenchmarkingChannel.Normal(0,0);
    public static AsymmetricTrustSystem ATS = new FiveATS();
    ...
    private long cur_timer_id;


    public RecPbftClient() {
    ...
    public void init(Properties properties) throws HandlerRegistrationException, IOException {
        ...
        cur_timer_id=setupTimer(new ComplainTimer(),10000);


    }

    //restart benchmark instance
    void uponComplainTimer(ComplainTimer t, long id){
        System.out.println("stuck");
        for(Proc q : peers){
            sendMessage("shutdown",new HashMap<>(),q,REPLICAID);
        }
        for(Process q : osc.allProcs){
            try {
                q.waitFor();
    ...
    private void uponIdUpdate(ProtoReply protoReply, short i) {
        HashMap<Proc,Proc> per = AsymmetricTrustSystem.random_permutation(osc.members);
        osc.ts = new ToleratedSystem(osc.ts.permute(per));
        ATS = ATS.permute(per);
        ats = ats.permute(per);

        faults = osc.computeFaults();
        osc.APBFT_One_shot_consensus_toy_ats(DISTRIBUTION,cur_run,DELTA);
        sendRequest(new ReInitKeyRequest(),AuthenticatedChannel.PROTO_ID);
    }
    ...
    public static class OneShotConsensus {
        ...
        public void APBFT_One_shot_consensus_toy_ats(BenchmarkingChannel.Distribution distribution, int run_id, double delta ) {
            ...
            for(Proc p : members){
                ...
                pb.command("java", "-jar", "Jars/RecPbftReplica.jar");
    ...
    // NEW METHOD: resume last state from filesystem
    private void resumeState() {
        File atsDir = new File(OUTPUT_DIR, ATS.name());
        File replicaDir = new File(atsDir, REPLICANAME);
        if (!replicaDir.exists()) return;
        // find max repetition
        for (File repDir : replicaDir.listFiles(File::isDirectory)) {
            try {
                int rep = Integer.parseInt(repDir.getName());
                repetition = Math.max(repetition, rep);
            } catch (NumberFormatException ignored) {}
        }
        // in that directory, find the largest fault file
        File lastRep = new File(replicaDir, Integer.toString(repetition));
        if (!lastRep.exists()) return;
        for (File f : lastRep.listFiles((d, name) -> name.endsWith(".txt"))) {
            try {
                int fault = Integer.parseInt(f.getName().replace(".txt", ""));
                cur_fault = Math.max(cur_fault, fault);
            } catch (Exception ignored) {}
        }
        // derive cur_run from repetition and cur_fault
        cur_run = repetition * 2 + cur_fault;
        System.out.println("Resuming at repetition=" + repetition + ", cur_fault=" + cur_fault + ", cur_run=" + cur_run);
    }
    ...
    public static void main(String[] args) throws IOException, InvalidParameterException, ProtocolAlreadyExistsException, HandlerRegistrationException {
        // instantiate client to resume state
        RecPbftClient c = new RecPbftClient();
        c.resumeState(); // ADDED: compute cur_run before consensus start

        // use resumed run id
        int run_id = c.cur_run;
        HashMap<Proc,Proc> per = AsymmetricTrustSystem.random_permutation(ATS.getMembers());
        ATS = ATS.permute(per);
    ...
    public static class ComplainTimer extends ProtoTimer {
        public static final short TIMER_ID = 10;
