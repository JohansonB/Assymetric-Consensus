...
public class EpochStatePredicateTracker implements PredicateTracker {
    ...
    public EpochStatePredicateTracker(TrustSystem t_s, Proc cur_leader) {
        this.treeSet = new TreeSet<>();
        this.tupleMap = new HashMap<>();
        this.t_s = t_s;
        this.k_s = t_s.get_quorums().get_kernel_system();
        this.cur_leader = cur_leader;
        bound_quorum = t_s.get_quorums().get_marked();
    ...
    public void insert(Proc p, int key, String value, HashMap<Integer,String> ws) {
        ...
        if(key==0&&value.equals("")||(leader_proposed&&p.equals(cur_leader))){
            ...
            if (contained) {
                ...
                bind_quorums.put(newTuple,t_s.get_quorums().get_marked());
                bind_kernels.put(newTuple,k_s.get_marked());
