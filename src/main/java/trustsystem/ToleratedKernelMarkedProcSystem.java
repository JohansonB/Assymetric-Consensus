...
public class ToleratedKernelMarkedProcSystem extends MarkedProcSystem {
    AsymmetricTrustSystem ats;
    HashSet<Proc> seen = new HashSet<>();
    public ToleratedKernelMarkedProcSystem(AsymmetricTrustSystem ats) {
        this.ats = ats;
    }
    ...
    public boolean mark_proc(Proc p){
        ...
        seen.add(p);
        HashSet<Proc> comp = new HashSet<>(ats.getMembers());
        comp.removeAll(seen);
        HashSet<Proc> g = contains_guild(seen);
        if(g.isEmpty()){
            found_quorum = new ProcSet(seen);
            p_set_found = true;
        }
        return p_set_found;
